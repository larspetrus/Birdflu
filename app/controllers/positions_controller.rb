  ...
  def index  # === Routed action ===
    ...
    @list_items =
        if @algs_mode
          ...
          raw_algs = raw_algs_from_combos = []
          combos_allowed = @only_position && alg_set.present? && alg_set.applies_to(@only_position)

          if (@list_format.combos != 'none') && combos_allowed
            raw_algs_from_combos = @only_position.algs_in_set(alg_set, sortby: @list_format.sortby, limit: @list_format.lines)
            flash.now[:notification] = "No Combo algs found" if raw_algs_from_combos.empty?
          end

          if @list_format.combos != 'only' || raw_algs_from_combos.empty?
          ...
          combos_in_list = raw_algs_from_combos.present?
          case ((raw_algs.present? ? 1 : 0) + (raw_algs_from_combos.present? ? 2 : 0))
            when 0 # make sure we have a list
              []
            when 1 #'none'
              raw_algs
            when 2 #'only'
              raw_algs_from_combos.map { |alg| [alg] + alg.combo_algs_in(alg_set) }.flatten
            when 3 #'merge'
              raw_alg_ids = Set.new(raw_algs.map(&:id))
              raw_algs += raw_algs_from_combos.reject{|cra| raw_alg_ids.include?(cra.id) }

              has_combos = Set.new(raw_algs_from_combos.map(&:id))
